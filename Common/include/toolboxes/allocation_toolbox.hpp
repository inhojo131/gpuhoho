...
/*!
 * \brief Copy memory to/from GPU.
 * \param[in] dst Destination pointer.
 * \param[in] src Source pointer.
 * \param[in] size Number of bytes to copy.
 * \param[in] kind cudaMemcpy kind (default: host to device).
 */
#ifdef HAVE_CUDA
inline void gpu_memcpy(void* dst, const void* src, size_t size,
                       cudaMemcpyKind kind = cudaMemcpyHostToDevice) noexcept {
  gpuErrChk(cudaMemcpy(dst, src, size, kind));
}
#else
inline void gpu_memcpy(void*, const void*, size_t, int = 0) noexcept {}
#endif
/*!
